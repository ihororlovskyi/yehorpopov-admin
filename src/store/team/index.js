...
export default {
  ...
  actions: {
    ...
    createTeamMember ({ commit, getters }, payload) {
      ...
      let photo
      let key
      firebase.database().ref('team/list').push(item)
        .then((data) => {
          key = data.key
          return key
        })
        ...
        .then(fileData => {
          ...
          return firebase.database().ref('team/list').child(key).update({ photo: photo })
        })
        .then(() => {
    ...
    updateTeamMember ({ commit }, payload) {
      ...
      let photo
      let key
      firebase.database().ref('team/list').child(payload.id).update(updateObj)
        .then((data) => {
          key = payload.id
          return key
        })
    ...
    deleteTeamMember ({ commit }, payload) {
      firebase.database().ref('team/list').child(payload.id).remove()
        .then(() => {
